...
import { BarQuery, PieQuery } from "../../interfaces/interface";
...
export const AnalyseExpense = () => {
  ...
  const setNewPieData = (
    ...
    if (data) {
      const creditTransactions: PieNivoData[] = data.map(
        (transaction: PieData) => ({
          id: transaction._id,
  ...
  const setNewBarData = (data: BarData[] | undefined): void => {
    if (data) {
      const creditTransactions: BarNivoData[] = data.map(
        (transaction: BarData) => {
          let date = "";
      ...
      const convertToTimestamp = (date: string): number[] => {
        ...
        const dateInNumbers = dateDetails.map((dateDetail) =>
          Number(dateDetail)
        );
  ...
  return (
    <div>
      ...
      <div className="barWrapper">
        <div
          className="bar"
          style={{ width: `${250 + barData.length * 70}px` }}
        >
          <Bar
            data={barData}
            indexBy="date"
